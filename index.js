...
//Get ip
const ip = "192.168.1.6"//process.argv[2];
		...
		switch(cmd.name)
		{
			case "join":
				try {
					...
					//Add listener
					tm.event("participants", room.getParticipants());

					room.on("participant_activity",(updateParticipants = (participant) => {
						tm.event("participant.activity", participant);
					}));
					...
					for (let stream of streams)
						//Add it
						participant.addStream(stream.stream,stream.participant);
			...
			case "stopVideo":
					try{
						...
						if(participant){
							const incomingStreamData = participant.removeVideoIncoming();
							const finalSdp = room.removeVideoFromParticipantStream(incomingStreamData,participant);
							const answer = participant.init(data.sdp);
			...
			case "startVideo":
					try{
						...
						for(let stream of participant.getIncomingStreams()){
							addedTrack = participant.updateIncomingStreamTrack(stream,sdp.getFirstStream().getTracks(),sdp.getFirstStream());
							userStream = addedTrack;
							break;
						}
						const finalSdp = room.addTrackToParticipantStream(userStream,sdp.getFirstStream().getTracks(),addedTrack,participant);
						const answer = participant.init(data.sdp);
