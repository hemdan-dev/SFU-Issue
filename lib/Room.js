...
class Room
{
	...
	createParticipant(name) 
	{
		...
		participant.on('stream',(stream)=>{
			...
			for (let other of this.participants.values())
				...
				if (participant.getId()!=other.getId())
					...
					other.addStream(stream,participant.getInfo());
			//Get audio tracks
		...
		participant.on('stopped', () => {
			...
			this.emitter.emit("participant_activity",{activity:"participant_left",id:participant.id,name:participant.name});
		});
		...
		this.emitter.emit("participant_activity",{activity:"participant_joined",id:participant.id,name:participant.name});
		
		//Done
		return participant;
	}

	addTrackToParticipantStream(stream,tracks,addedTrack,currentpart){
		for (let participant of this.participants.values()){
			if (currentpart.getId()!=participant.getId()) participant.updateOutgoingStreamTrack(stream,tracks,addedTrack,currentpart);
		}
	}

	removeVideoFromParticipantStream(stream,currentpart){
		for (let participant of this.participants.values()){
			if (currentpart.getId()!=participant.getId()) participant.removeVideoOutgoing(stream,currentpart);
		}
	}
	...
	getStreams() {
		...
		for (let participant of this.participants.values())
			...
			for (let stream of participant.getIncomingStreams())
				...
				streams.push({stream:stream,participant:participant.getInfo()});
		//return them
		return streams;
	}

	getParticipants(){
		return this.getInfo().participants;
	}
